...
class CropBox extends StatefulWidget {
  ...
  final CustomPainter? extraPainter;
...
class _CropBoxState extends State<CropBox> {
  ...
  void caculateInitClipSize() {
    ...
    double _cropAspectRatio =
        _cropBoxRealSize.width / _cropBoxRealSize.height; //裁剪框宽高比
    double _clipAspectRatio =
        _originClipSize.width / _originClipSize.height; //素材宽高比
  ...
  void caculateInitClipPosition() {
    ...
    if (resultRect == Rect.fromLTRB(0, 0, 1, 1)) {
      ...
      double _clipAspectRatio =
          _resizeClipSize.width / _resizeClipSize.height; //素材宽高比
      double _cropAspectRatio =
          _cropBoxRealSize.width / _cropBoxRealSize.height; //裁剪区域宽高比
      Rect _tempRect;
      if (_cropAspectRatio > _clipAspectRatio) {
        // 如果裁剪框宽高比大于素材宽高比
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
        future: _loading,
        builder: (_, snapshot) {
          final extraPaiter = widget.extraPainter;
                  ...
                  child: (isReady &&
                      ...
                      ? Stack(
                          children: [
                            ...
                            extraPaiter != null
                                ? Positioned.fromRect(
                                    rect: _cropBoxRealRect,
                                    child: CustomPaint(
                                      size: _cropBoxRealRect.size,
                                      painter: extraPaiter,
                                    ),
                                  )
                                : Container(),
